...
class AddressingModeSpec: QuickSpec {
    override func spec() {
        ...
        describe("CPU.fetchOperand") {
            ...
            context("implicit") {
                it("return 0") {
                    let (operand, pc) = cpu.measurePC(Operand.implicit)
            ...
            context("accumulator") {
                it("return data on accumulator") {
                    ...
                    let (operand, pc) = cpu.measurePC(Operand.accumulator)
            ...
            context("immediate") {
                it("return PC's data directly") {
                    let (operand, pc) = cpu.measurePC(Operand.immediate)
            ...
            context("zeroPage") {
                it("return 8 bit operand at addressing by PC on memory") {
                    let (operand, pc) = cpu.measurePC(Operand.zeroPage)
            ...
            context("zeroPageX") {
                it("return 8 bit operand at addressing by PC added X on memory") {
                    let (operand, pc) = cpu.measurePC(Operand.zeroPageX)
            ...
            context("zeroPageY") {
                it("return 8 bit operand at addressing by PC added Y on memory") {
                    let (operand, pc) = cpu.measurePC(Operand.zeroPageY)
            ...
            context("absolute") {
                it("return full 16 bit address") {
                    let (operand, pc) = cpu.measurePC(Operand.absolute)
            ...
            context("absoluteX") {
                it("return full 16 bit address added X") {
                    let (operand, pc) = cpu.measurePC(Operand.absoluteX)
            ...
            context("absoluteY") {
                it("return full 16 bit address added Y") {
                    let (operand, pc) = cpu.measurePC(Operand.absoluteY)
            ...
            context("relative") {
                it("return offset") {
                    ...
                    let (operand, pc) = cpu.measurePC(Operand.relative)
            ...
            context("indirect") {
                it("return (Indirect) address") {
                    let (operand, pc) = cpu.measurePC(Operand.indirect)
            ...
            context("indexedIndirect") {
                it("return (Indirect, X) address") {
                    ...
                    let (operand, pc) = cpu.measurePC(Operand.indexedIndirect)
            ...
            context("indirectIndexed") {
                it("return (Indirect), Y address") {
                    ...
                    let (operand, pc) = cpu.measurePC(Operand.indirectIndexed)
