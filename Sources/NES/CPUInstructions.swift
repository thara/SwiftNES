...
extension AddressingMode where Self: CPURegisters & CPUMemory & CPUCycle {
    ...
    @inline(__always)
    mutating func zeroPage() -> UInt16 {
        let operand = cpuRead(at: PC).u16 & 0xFF
        PC &+= 1
        return operand
    }
    ...
    mutating func zeroPageX() -> UInt16 {
        ...
        let operand = (cpuRead(at: PC).u16 &+ X.u16) & 0xFF
        PC &+= 1
        return operand
    }
    ...
    mutating func zeroPageY() -> UInt16 {
        ...
        let operand = (cpuRead(at: PC).u16 &+ Y.u16) & 0xFF
        PC &+= 1
        return operand
    }
    ...
    mutating func readOnIndirect(operand: UInt16) -> UInt16 {
        let low = cpuRead(at: operand).u16
        let high = cpuRead(at: operand & 0xFF00 | ((operand &+ 1) & 0x00FF)).u16 &<< 8  // Reproduce 6502 bug; http://nesdev.com/6502bugs.txt
        return low | high
    }
}
...
extension CPUInstruction where Self: CPUCycle & CPURegisters & CPUMemory & CPUStack & AddressingMode {

    mutating func fetchOperand() -> OpCode {
        let opcode = cpuRead(at: PC)
        PC &+= 1
        return opcode
    }
    ...
    mutating func STX(operand: Operand) {
        cpuWrite(X, at: operand)
    }

    /// storeYRegister
    mutating func STY(operand: Operand) {
        cpuWrite(Y, at: operand)
    }
    ...
    mutating func CMP(operand: Operand) {
        let cmp = Int16(A) &- Int16(cpuRead(at: operand))
    ...
    mutating func INC(operand: Operand) {
        let result = cpuRead(at: operand) &+ 1

        P.setZN(result)
        cpuWrite(result, at: operand)
