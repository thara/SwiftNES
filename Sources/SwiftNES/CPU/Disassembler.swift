...
class Disassembler {
    ...
    func disassemble(cpu: inout CPU, memory: inout Memory) -> (machineCode: String, assemblyCode: String) {
        let currentStep = makeCurrentStep(cpu: &cpu, memory: &memory)
        let opcode = memory.read(at: currentStep.pc)
        let instruction = instructionTable[Int(opcode)]
        return (
            makeMachineCode(step: currentStep, instruction: instruction),
            makeAssemblyCode(step: currentStep, instruction: instruction, memory: &memory))
    }

    private func makeCurrentStep(cpu: inout CPU, memory: inout Memory) -> CPUStep {
        let pc = cpu.PC
        return CPUStep(
            pc: cpu.PC,
            operand1: memory.read(at: pc &+ 1),
            operand2: memory.read(at: pc &+ 2),
            state: cpu
        )
    }
    ...
    private func makeAssemblyCode(step: CPUStep, instruction: Instruction, memory: inout Memory) -> String {
        let operandString = makeAssemblyOperand(step: step, instruction: instruction, memory: &memory)
        let prefix = undocumentedOpcodes.contains(Int(instruction.opcode)) ? "*" : " "
        return "\(prefix)\(instruction.mnemonic) \(operandString)"
    }

    private func makeAssemblyOperand(step: CPUStep, instruction: Instruction, memory: inout Memory) -> String {
        switch instruction.mnemonic {
        case .JMP, .JSR:
            if case .absolute = instruction.addressingMode {
                let address = decodeAddress(step: step, addressingMode: instruction.addressingMode, memory: &memory)
        ...
        return makeAssemblyOperand(step: step, addressingMode: instruction.addressingMode, memory: &memory)
    }

    // swiftlint:disable cyclomatic_complexity line_length
    private func makeAssemblyOperand(step: CPUStep, addressingMode: AddressingMode, memory: inout Memory) -> String {
        ...
        case .zeroPageX:
            return String(format: "$%02X,X @ %02X = %02X", operand1, operand1 &+ x, memory.read(at: (operand1 &+ x).u16))
        case .zeroPageY:
            return String(format: "$%02X,Y @ %02X = %02X", operand1, operand1 &+ y, memory.read(at: (operand1 &+ y).u16))
        case .absolute:
            return String(format: "$%04X = %02X", operand16, memory.read(at: operand16))
        case .absoluteX:
            return String(format: "$%04X,X @ %04X = %02X", operand16, operand16 &+ x.u16, memory.read(at: operand16 &+ x.u16))
        case .absoluteY:
            return String(format: "$%04X,Y @ %04X = %02X", operand16, operand16 &+ y.u16, memory.read(at: operand16 &+ y.u16))
        case .relative:
    ...
    private func decodeAddress(step: CPUStep, addressingMode: AddressingMode, memory: inout Memory) -> UInt16 {
        switch addressingMode {
        case .implicit:
            return 0x00
        case .immediate:
